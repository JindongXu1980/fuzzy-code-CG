function [center, U_u,U_l, obj_fcn] = FCMClust(udata,ldata,lable,cluster_n,t0)
...
% Main loop  主要循环
A=ones(1,cluster_n);
AA=zeros(1,cluster_n);
for i = 1:max_iter,
   
    [U_ulow,U_uup] = cu(center,cluster_n,udata,expo);        % 构建待测样本的隶属度区间
   if sum(abs(A-AA))>0.001
     AA=A;
    [U_u,A]=changeait2fm(U_ulow,U_uup,U_u,udata,center,expo,i);  % 待测样本隶属度区间自适应降型
    A(isnan(A)) = 0;
   else
%        fprintf('%d\n',i)
       t1=cputime-t0;
%        break;
       [U_u]=changeait2fm_2(U_ulow,U_uup,U_u,udata,A);
   end
