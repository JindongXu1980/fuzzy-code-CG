function [center, U, obj_fcn] = FCMClustnew(x,data, cluster_n, options)
...
if nargin ~= 3 & nargin ~= 4,    %判断输入参数个数只能是3个或4个
...
% 默认操作参数
default_options = [2.0; % 隶属度矩阵U的指数
    400;                % 最大迭代次数 
    1e-6;               % 隶属度最小变化量,迭代终止条件
    1];                 % 每次迭代是否输出信息标志 
...
for i = 1:max_iter,
 ...
 %[U, center, obj_fcn(i)] =stepfcmnew(data, U, cluster_n,expo, x,mymean,mystd);
  [U, center, obj_fcn(i)] =stepfcmnew(data, U, cluster_n,expo);
